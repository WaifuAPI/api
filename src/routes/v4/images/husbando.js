...
import getHusbando from '../../../controllers/v4/images/husbando.js';
...
router
  .route('/')
  /**
   * @api {get} v4/husbando Get a Random Husbando
   * @apiDescription Retrieve a random Husbando.
   * @apiName getHusbando
   * @apiGroup images
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getHusbando)
  /**
   * @api {post} v4/husbando Increment Husbando Data
   * @apiDescription Increment data related to Husbandos (only accessible by database moderators).
   * @apiName IncrementHusbandoData
   * @apiGroup images
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Husbando'));

router
  .route('/:id')
  /**
   * @api {patch} v4/husbando/:id Update Husbando Data
   * @apiDescription Update data related to Husbandos with a specific ID (only accessible by database moderators).
   * @apiName UpdateHusbandoData
   * @apiGroup Husbando
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Husbando'))
  /**
   * @api {delete} v4/husbando/:id Delete Husbando Data
   * @apiDescription Delete data related to Husbando with a specific ID (only accessible by admins).
   * @apiName DeleteHusbandoData
   * @apiGroup Husbando
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Husbando'));
// Export the router
