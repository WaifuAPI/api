...
import getRandomNope from '../../../controllers/v4/interactions/nope.js';
...
router
  .route('/')
  /**
   * @api {get} v4/nope Get a Random Nope
   * @apiDescription Retrieve a random nope.
   * @apiName GetRandomNope
   * @apiGroup Nope
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomNope)
  /**
   * @api {post} v4/nope Increment Nope Data
   * @apiDescription Increment data related to nopes (only accessible by database moderators).
   * @apiName IncrementNopeData
   * @apiGroup Nope
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Nope'));

router
  .route('/:id')
  /**
   * @api {patch} v4/nope/:id Update Nope Data
   * @apiDescription Update data related to nopes with a specific ID (only accessible by database moderators).
   * @apiName UpdateNopeData
   * @apiGroup Nope
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Nope'))
  /**
   * @api {delete} v4/nope/:id Delete Nope Data
   * @apiDescription Delete data related to nope with a specific ID (only accessible by admins).
   * @apiName DeleteNopeData
   * @apiGroup Nope
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Nope'));
// Export the router
