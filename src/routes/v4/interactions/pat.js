...
import getRandomPat from '../../../controllers/v4/interactions/pat.js';
...
router
  .route('/')
  /**
   * @api {get} v4/pat Get a Random Pat
   * @apiDescription Retrieve a random pat.
   * @apiName GetRandomPat
   * @apiGroup Pat
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomPat)
  /**
   * @api {post} v4/pat Increment Pat Data
   * @apiDescription Increment data related to pats (only accessible by database moderators).
   * @apiName IncrementPatData
   * @apiGroup Pat
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Pat'));

router
  .route('/:id')
  /**
   * @api {patch} v4/pat/:id Update Pat Data
   * @apiDescription Update data related to pats with a specific ID (only accessible by database moderators).
   * @apiName UpdatePatData
   * @apiGroup Pat
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Pat'))
  /**
   * @api {delete} v4/pat/:id Delete Pat Data
   * @apiDescription Delete data related to pat with a specific ID (only accessible by admins).
   * @apiName DeletePatData
   * @apiGroup Pat
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Pat'));
// Export the router
