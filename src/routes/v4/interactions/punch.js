...
import getRandomPunch from '../../../controllers/v4/interactions/punch.js';
...
router
  .route('/')
  /**
   * @api {get} v4/punch Get a Random Punch
   * @apiDescription Retrieve a random punch.
   * @apiName GetRandomPunch
   * @apiGroup Punch
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomPunch)
  /**
   * @api {post} v4/punch Increment Punch Data
   * @apiDescription Increment data related to punchs (only accessible by database moderators).
   * @apiName IncrementPunchData
   * @apiGroup Punch
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Punch'));

router
  .route('/:id')
  /**
   * @api {patch} v4/punch/:id Update Punch Data
   * @apiDescription Update data related to punchs with a specific ID (only accessible by database moderators).
   * @apiName UpdatePunchData
   * @apiGroup Punch
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Punch'))
  /**
   * @api {delete} v4/punch/:id Delete Punch Data
   * @apiDescription Delete data related to punch with a specific ID (only accessible by admins).
   * @apiName DeletePunchData
   * @apiGroup Punch
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Punch'));
// Export the router
