...
import getRandomHighfive from '../../../controllers/v4/interactions/highfive.js';
...
router
  .route('/')
  /**
   * @api {get} v4/highfive Get a Random Highfive
   * @apiDescription Retrieve a random highfive.
   * @apiName GetRandomHighfive
   * @apiGroup Highfive
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomHighfive)
  /**
   * @api {post} v4/highfive Increment Highfive Data
   * @apiDescription Increment data related to highfives (only accessible by database moderators).
   * @apiName IncrementHighfiveData
   * @apiGroup Highfive
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Highfive'));

router
  .route('/:id')
  /**
   * @api {patch} v4/highfive/:id Update Highfive Data
   * @apiDescription Update data related to highfives with a specific ID (only accessible by database moderators).
   * @apiName UpdateHighfiveData
   * @apiGroup Highfive
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Highfive'))
  /**
   * @api {delete} v4/highfive/:id Delete Highfive Data
   * @apiDescription Delete data related to highfive with a specific ID (only accessible by admins).
   * @apiName DeleteHighfiveData
   * @apiGroup Highfive
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Highfive'));
// Export the router
