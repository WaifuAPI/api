...
import getRandomCuddle from '../../../controllers/v4/interactions/cuddle.js';
...
router
  .route('/')
  /**
   * @api {get} v4/cuddle Get a Random Cuddle
   * @apiDescription Retrieve a random cuddle.
   * @apiName GetRandomCuddle
   * @apiGroup Cuddle
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomCuddle)
  /**
   * @api {post} v4/cuddle Increment Cuddle Data
   * @apiDescription Increment data related to cuddles (only accessible by database moderators).
   * @apiName IncrementCuddleData
   * @apiGroup Cuddle
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Cuddle'));

router
  .route('/:id')
  /**
   * @api {patch} v4/cuddle/:id Update Cuddle Data
   * @apiDescription Update data related to cuddles with a specific ID (only accessible by database moderators).
   * @apiName UpdateCuddleData
   * @apiGroup Cuddle
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Cuddle'))
  /**
   * @api {delete} v4/cuddle/:id Delete Cuddle Data
   * @apiDescription Delete data related to cuddle with a specific ID (only accessible by admins).
   * @apiName DeleteCuddleData
   * @apiGroup Cuddle
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Cuddle'));
// Export the router
