...
import getRandomBonk from '../../../controllers/v4/interactions/bonk.js';
...
router
  .route('/')
  /**
   * @api {get} v4/bonk Get a Random Bonk
   * @apiDescription Retrieve a random bonk.
   * @apiName GetRandomBonk
   * @apiGroup Bonk
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomBonk)
  /**
   * @api {post} v4/bonk Increment Bonk Data
   * @apiDescription Increment data related to bonks (only accessible by database moderators).
   * @apiName IncrementBonkData
   * @apiGroup Bonk
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Bonk'));

router
  .route('/:id')
  /**
   * @api {patch} v4/bonk/:id Update Bonk Data
   * @apiDescription Update data related to bonks with a specific ID (only accessible by database moderators).
   * @apiName UpdateBonkData
   * @apiGroup Bonk
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Bonk'))
  /**
   * @api {delete} v4/bonk/:id Delete Bonk Data
   * @apiDescription Delete data related to bonk with a specific ID (only accessible by admins).
   * @apiName DeleteBonkData
   * @apiGroup Bonk
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Bonk'));
// Export the router
