...
import getRandomKick from '../../../controllers/v4/interactions/kick.js';
...
router
  .route('/')
  /**
   * @api {get} v4/kick Get a Random Kick
   * @apiDescription Retrieve a random kick.
   * @apiName GetRandomKick
   * @apiGroup Kick
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomKick)
  /**
   * @api {post} v4/kick Increment Kick Data
   * @apiDescription Increment data related to kicks (only accessible by database moderators).
   * @apiName IncrementKickData
   * @apiGroup Kick
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Kick'));

router
  .route('/:id')
  /**
   * @api {patch} v4/kick/:id Update Kick Data
   * @apiDescription Update data related to kicks with a specific ID (only accessible by database moderators).
   * @apiName UpdateKickData
   * @apiGroup Kick
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Kick'))
  /**
   * @api {delete} v4/kick/:id Delete Kick Data
   * @apiDescription Delete data related to kick with a specific ID (only accessible by admins).
   * @apiName DeleteKickData
   * @apiGroup Kick
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Kick'));
// Export the router
