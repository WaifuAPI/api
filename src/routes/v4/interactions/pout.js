...
import getRandomPout from '../../../controllers/v4/interactions/pout.js';
...
router
  .route('/')
  /**
   * @api {get} v4/pout Get a Random Pout
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomPout)
  /**
   * @api {post} v4/pout Increment Pout Data
   * @apiDescription Increment data related to pouts (only accessible by database moderators).
   * @apiName IncrementPoutData
   * @apiGroup Pout
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Pout'));

router
  .route('/:id')
  /**
   * @api {patch} v4/pout/:id Update Pout Data
   * @apiDescription Update data related to pouts with a specific ID (only accessible by database moderators).
   * @apiName UpdatePoutData
   * @apiGroup Pout
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Pout'))
  /**
   * @api {delete} v4/pout/:id Delete Pout Data
   * @apiDescription Delete data related to pout with a specific ID (only accessible by admins).
   * @apiName DeletePoutData
   * @apiGroup Pout
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Pout'));
// Export the router
