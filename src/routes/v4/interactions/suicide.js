...
import getRandomSuicide from '../../../controllers/v4/interactions/suicide.js';
...
router
  .route('/')
  /**
   * @api {get} v4/suicide Get a Random Suicide
   * @apiDescription Retrieve a random suicide.
   * @apiName GetRandomSuicide
   * @apiGroup Suicide
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomSuicide)
  /**
   * @api {post} v4/suicide Increment Suicide Data
   * @apiDescription Increment data related to suicides (only accessible by database moderators).
   * @apiName IncrementSuicideData
   * @apiGroup Suicide
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Suicide'));

router
  .route('/:id')
  /**
   * @api {patch} v4/suicide/:id Update Suicide Data
   * @apiDescription Update data related to suicides with a specific ID (only accessible by database moderators).
   * @apiName UpdateSuicideData
   * @apiGroup Suicide
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Suicide'))
  /**
   * @api {delete} v4/suicide/:id Delete Suicide Data
   * @apiDescription Delete data related to suicide with a specific ID (only accessible by admins).
   * @apiName DeleteSuicideData
   * @apiGroup Suicide
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Suicide'));
// Export the router
