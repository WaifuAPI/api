...
import getRandomBully from '../../../controllers/v4/interactions/bully.js';
...
router
  .route('/')
  /**
   * @api {get} v4/bully Get a Random Bully
   * @apiDescription Retrieve a random bully.
   * @apiName GetRandomBully
   * @apiGroup Bully
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomBully)
  /**
   * @api {post} v4/bully Increment Bully Data
   * @apiDescription Increment data related to bullys (only accessible by database moderators).
   * @apiName IncrementBullyData
   * @apiGroup Bully
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Bully'));

router
  .route('/:id')
  /**
   * @api {patch} v4/bully/:id Update Bully Data
   * @apiDescription Update data related to bullys with a specific ID (only accessible by database moderators).
   * @apiName UpdateBullyData
   * @apiGroup Bully
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Bully'))
  /**
   * @api {delete} v4/bully/:id Delete Bully Data
   * @apiDescription Delete data related to bully with a specific ID (only accessible by admins).
   * @apiName DeleteBullyData
   * @apiGroup Bully
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Bully'));
// Export the router
