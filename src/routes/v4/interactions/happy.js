...
import getRandomHappy from '../../../controllers/v4/interactions/happy.js';
...
router
  .route('/')
  /**
   * @api {get} v4/happy Get a Random Happy
   * @apiDescription Retrieve a random happy.
   * @apiName GetRandomHappy
   * @apiGroup Happy
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomHappy)
  /**
   * @api {post} v4/happy Increment Happy Data
   * @apiDescription Increment data related to happys (only accessible by database moderators).
   * @apiName IncrementHappyData
   * @apiGroup Happy
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Happy'));

router
  .route('/:id')
  /**
   * @api {patch} v4/happy/:id Update Happy Data
   * @apiDescription Update data related to happys with a specific ID (only accessible by database moderators).
   * @apiName UpdateHappyData
   * @apiGroup Happy
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Happy'))
  /**
   * @api {delete} v4/happy/:id Delete Happy Data
   * @apiDescription Delete data related to happy with a specific ID (only accessible by admins).
   * @apiName DeleteHappyData
   * @apiGroup Happy
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Happy'));
// Export the router
