...
import getRandomNuzzle from '../../../controllers/v4/interactions/nuzzle.js';
...
router
  .route('/')
  /**
   * @api {get} v4/nuzzle Get a Random Nuzzle
   * @apiDescription Retrieve a random nuzzle.
   * @apiName GetRandomNuzzle
   * @apiGroup Nuzzle
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomNuzzle)
  /**
   * @api {post} v4/nuzzle Increment Nuzzle Data
   * @apiDescription Increment data related to nuzzles (only accessible by database moderators).
   * @apiName IncrementNuzzleData
   * @apiGroup Nuzzle
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Nuzzle'));

router
  .route('/:id')
  /**
   * @api {patch} v4/nuzzle/:id Update Nuzzle Data
   * @apiDescription Update data related to nuzzles with a specific ID (only accessible by database moderators).
   * @apiName UpdateNuzzleData
   * @apiGroup Nuzzle
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Nuzzle'))
  /**
   * @api {delete} v4/nuzzle/:id Delete Nuzzle Data
   * @apiDescription Delete data related to nuzzle with a specific ID (only accessible by admins).
   * @apiName DeleteNuzzleData
   * @apiGroup Nuzzle
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Nuzzle'));
// Export the router
