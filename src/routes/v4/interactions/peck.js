...
import getRandomPeck from '../../../controllers/v4/interactions/peck.js';
...
router
  .route('/')
  /**
   * @api {get} v4/peck Get a Random Peck
   * @apiDescription Retrieve a random peck.
   * @apiName GetRandomPeck
   * @apiGroup Peck
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomPeck)
  /**
   * @api {post} v4/peck Increment Peck Data
   * @apiDescription Increment data related to pecks (only accessible by database moderators).
   * @apiName IncrementPeckData
   * @apiGroup Peck
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Peck'));

router
  .route('/:id')
  /**
   * @api {patch} v4/peck/:id Update Peck Data
   * @apiDescription Update data related to pecks with a specific ID (only accessible by database moderators).
   * @apiName UpdatePeckData
   * @apiGroup Peck
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Peck'))
  /**
   * @api {delete} v4/peck/:id Delete Peck Data
   * @apiDescription Delete data related to peck with a specific ID (only accessible by admins).
   * @apiName DeletePeckData
   * @apiGroup Peck
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Peck'));
// Export the router
