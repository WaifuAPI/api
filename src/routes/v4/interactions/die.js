...
import getRandomDie from '../../../controllers/v4/interactions/die.js';
...
router
  .route('/')
  /**
   * @api {get} v4/die Get a Random Die
   * @apiDescription Retrieve a random die.
   * @apiName GetRandomDie
   * @apiGroup Die
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomDie)
  /**
   * @api {post} v4/die Increment Die Data
   * @apiDescription Increment data related to dies (only accessible by database moderators).
   * @apiName IncrementDieData
   * @apiGroup Die
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Die'));

router
  .route('/:id')
  /**
   * @api {patch} v4/die/:id Update Die Data
   * @apiDescription Update data related to dies with a specific ID (only accessible by database moderators).
   * @apiName UpdateDieData
   * @apiGroup Die
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Die'))
  /**
   * @api {delete} v4/die/:id Delete Die Data
   * @apiDescription Delete data related to die with a specific ID (only accessible by admins).
   * @apiName DeleteDieData
   * @apiGroup Die
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Die'));
// Export the router
