...
import getRandomStab from '../../../controllers/v4/interactions/stab.js';
...
router
  .route('/')
  /**
   * @api {get} v4/stab Get a Random Stab
   * @apiDescription Retrieve a random stab.
   * @apiName GetRandomStab
   * @apiGroup Stab
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomStab)
  /**
   * @api {post} v4/stab Increment Stab Data
   * @apiDescription Increment data related to stabs (only accessible by database moderators).
   * @apiName IncrementStabData
   * @apiGroup Stab
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Stab'));

router
  .route('/:id')
  /**
   * @api {patch} v4/stab/:id Update Stab Data
   * @apiDescription Update data related to stabs with a specific ID (only accessible by database moderators).
   * @apiName UpdateStabData
   * @apiGroup Stab
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Stab'))
  /**
   * @api {delete} v4/stab/:id Delete Stab Data
   * @apiDescription Delete data related to stab with a specific ID (only accessible by admins).
   * @apiName DeleteStabData
   * @apiGroup Stab
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Stab'));
// Export the router
