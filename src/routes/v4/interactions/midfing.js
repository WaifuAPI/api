...
import getRandomMidfing from '../../../controllers/v4/interactions/midfing.js';
...
router
  .route('/')
  /**
   * @api {get} v4/midfing Get a Random Midfing
   * @apiDescription Retrieve a random midfing.
   * @apiName GetRandomMidfing
   * @apiGroup Midfing
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomMidfing)
  /**
   * @api {post} v4/midfing Increment Midfing Data
   * @apiDescription Increment data related to midfings (only accessible by database moderators).
   * @apiName IncrementMidfingData
   * @apiGroup Midfing
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Midfing'));

router
  .route('/:id')
  /**
   * @api {patch} v4/midfing/:id Update Midfing Data
   * @apiDescription Update data related to midfings with a specific ID (only accessible by database moderators).
   * @apiName UpdateMidfingData
   * @apiGroup Midfing
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Midfing'))
  /**
   * @api {delete} v4/midfing/:id Delete Midfing Data
   * @apiDescription Delete data related to midfing with a specific ID (only accessible by admins).
   * @apiName DeleteMidfingData
   * @apiGroup Midfing
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Midfing'));
// Export the router
