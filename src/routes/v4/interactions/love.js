...
import getRandomLove from '../../../controllers/v4/interactions/love.js';
...
router
  .route('/')
  /**
   * @api {get} v4/love Get a Random Love
   * @apiDescription Retrieve a random love.
   * @apiName GetRandomLove
   * @apiGroup Love
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomLove)
  /**
   * @api {post} v4/love Increment Love Data
   * @apiDescription Increment data related to loves (only accessible by database moderators).
   * @apiName IncrementLoveData
   * @apiGroup Love
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Love'));

router
  .route('/:id')
  /**
   * @api {patch} v4/love/:id Update Love Data
   * @apiDescription Update data related to loves with a specific ID (only accessible by database moderators).
   * @apiName UpdateLoveData
   * @apiGroup Love
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Love'))
  /**
   * @api {delete} v4/love/:id Delete Love Data
   * @apiDescription Delete data related to love with a specific ID (only accessible by admins).
   * @apiName DeleteLoveData
   * @apiGroup Love
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Love'));
// Export the router
