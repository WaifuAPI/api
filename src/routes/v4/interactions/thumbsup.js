...
import getRandomThumbsup from '../../../controllers/v4/interactions/thumbsup.js';
...
router
  .route('/')
  /**
   * @api {get} v4/thumbsup Get a Random Thumbsup
   * @apiDescription Retrieve a random thumbsup.
   * @apiName GetRandomThumbsup
   * @apiGroup Thumbsup
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomThumbsup)
  /**
   * @api {post} v4/thumbsup Increment Thumbsup Data
   * @apiDescription Increment data related to thumbsups (only accessible by database moderators).
   * @apiName IncrementThumbsupData
   * @apiGroup Thumbsup
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Thumbsup'));

router
  .route('/:id')
  /**
   * @api {patch} v4/thumbsup/:id Update Thumbsup Data
   * @apiDescription Update data related to thumbsups with a specific ID (only accessible by database moderators).
   * @apiName UpdateThumbsupData
   * @apiGroup Thumbsup
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Thumbsup'))
  /**
   * @api {delete} v4/thumbsup/:id Delete Thumbsup Data
   * @apiDescription Delete data related to thumbsup with a specific ID (only accessible by admins).
   * @apiName DeleteThumbsupData
   * @apiGroup Thumbsup
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Thumbsup'));
// Export the router
