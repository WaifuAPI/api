...
import getRandomKiss from '../../../controllers/v4/interactions/kiss.js';
...
router
  .route('/')
  /**
   * @api {get} v4/kiss Get a Random Kiss
   * @apiDescription Retrieve a random kiss.
   * @apiName GetRandomKiss
   * @apiGroup Kiss
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomKiss)
  /**
   * @api {post} v4/kiss Increment Kiss Data
   * @apiDescription Increment data related to kisss (only accessible by database moderators).
   * @apiName IncrementKissData
   * @apiGroup Kiss
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Kissu'));

router
  .route('/:id')
  /**
   * @api {patch} v4/kiss/:id Update Kiss Data
   * @apiDescription Update data related to kisss with a specific ID (only accessible by database moderators).
   * @apiName UpdateKissData
   * @apiGroup Kiss
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Kissu'))
  /**
   * @api {delete} v4/kiss/:id Delete Kiss Data
   * @apiDescription Delete data related to kiss with a specific ID (only accessible by admins).
   * @apiName DeleteKissData
   * @apiGroup Kiss
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Kissu'));
// Export the router
