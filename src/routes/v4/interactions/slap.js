...
import getRandomSlap from '../../../controllers/v4/interactions/slap.js';
...
router
  .route('/')
  /**
   * @api {get} v4/slap Get a Random Slap
   * @apiDescription Retrieve a random slap.
   * @apiName GetRandomSlap
   * @apiGroup Slap
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomSlap)
  /**
   * @api {post} v4/slap Increment Slap Data
   * @apiDescription Increment data related to slaps (only accessible by database moderators).
   * @apiName IncrementSlapData
   * @apiGroup Slap
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Slap'));

router
  .route('/:id')
  /**
   * @api {patch} v4/slap/:id Update Slap Data
   * @apiDescription Update data related to slaps with a specific ID (only accessible by database moderators).
   * @apiName UpdateSlapData
   * @apiGroup Slap
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Slap'))
  /**
   * @api {delete} v4/slap/:id Delete Slap Data
   * @apiDescription Delete data related to slap with a specific ID (only accessible by admins).
   * @apiName DeleteSlapData
   * @apiGroup Slap
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Slap'));
// Export the router
