...
import getRandomLaugh from '../../../controllers/v4/interactions/laugh.js';
...
router
  .route('/')
  /**
   * @api {get} v4/laugh Get a Random Laugh
   * @apiDescription Retrieve a random laugh.
   * @apiName GetRandomLaugh
   * @apiGroup Laugh
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomLaugh)
  /**
   * @api {post} v4/laugh Increment Laugh Data
   * @apiDescription Increment data related to laughs (only accessible by database moderators).
   * @apiName IncrementLaughData
   * @apiGroup Laugh
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Laugh'));

router
  .route('/:id')
  /**
   * @api {patch} v4/laugh/:id Update Laugh Data
   * @apiDescription Update data related to laughs with a specific ID (only accessible by database moderators).
   * @apiName UpdateLaughData
   * @apiGroup Laugh
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Laugh'))
  /**
   * @api {delete} v4/laugh/:id Delete Laugh Data
   * @apiDescription Delete data related to laugh with a specific ID (only accessible by admins).
   * @apiName DeleteLaughData
   * @apiGroup Laugh
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Laugh'));
// Export the router
