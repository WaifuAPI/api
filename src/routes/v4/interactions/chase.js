...
import getRandomChase from '../../../controllers/v4/interactions/chase.js';
...
router
  .route('/')
  /**
   * @api {get} v4/chase Get a Random Chase
   * @apiDescription Retrieve a random chase.
   * @apiName GetRandomChase
   * @apiGroup Chase
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomChase)
  /**
   * @api {post} v4/chase Increment Chase Data
   * @apiDescription Increment data related to chases (only accessible by database moderators).
   * @apiName IncrementChaseData
   * @apiGroup Chase
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Chase'));

router
  .route('/:id')
  /**
   * @api {patch} v4/chase/:id Update Chase Data
   * @apiDescription Update data related to chases with a specific ID (only accessible by database moderators).
   * @apiName UpdateChaseData
   * @apiGroup Chase
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Chase'))
  /**
   * @api {delete} v4/chase/:id Delete Chase Data
   * @apiDescription Delete data related to chase with a specific ID (only accessible by admins).
   * @apiName DeleteChaseData
   * @apiGroup Chase
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Chase'));
// Export the router
