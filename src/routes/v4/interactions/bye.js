...
import getRandomBye from '../../../controllers/v4/interactions/bye.js';
...
router
  .route('/')
  /**
   * @api {get} v4/bye Get a Random Bye
   * @apiDescription Retrieve a random bye.
   * @apiName GetRandomBye
   * @apiGroup Bye
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomBye)
  /**
   * @api {post} v4/bye Increment Bye Data
   * @apiDescription Increment data related to byes (only accessible by database moderators).
   * @apiName IncrementByeData
   * @apiGroup Bye
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Bye'));

router
  .route('/:id')
  /**
   * @api {patch} v4/bye/:id Update Bye Data
   * @apiDescription Update data related to byes with a specific ID (only accessible by database moderators).
   * @apiName UpdateByeData
   * @apiGroup Bye
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Bye'))
  /**
   * @api {delete} v4/bye/:id Delete Bye Data
   * @apiDescription Delete data related to bye with a specific ID (only accessible by admins).
   * @apiName DeleteByeData
   * @apiGroup Bye
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Bye'));
// Export the router
