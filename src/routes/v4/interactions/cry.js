...
import getRandomCry from '../../../controllers/v4/interactions/cry.js';
...
router
  .route('/')
  /**
   * @api {get} v4/cry Get a Random Cry
   * @apiDescription Retrieve a random cry.
   * @apiName GetRandomCry
   * @apiGroup Cry
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomCry)
  /**
   * @api {post} v4/cry Increment Cry Data
   * @apiDescription Increment data related to crys (only accessible by database moderators).
   * @apiName IncrementCryData
   * @apiGroup Cry
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Cry'));

router
  .route('/:id')
  /**
   * @api {patch} v4/cry/:id Update Cry Data
   * @apiDescription Update data related to crys with a specific ID (only accessible by database moderators).
   * @apiName UpdateCryData
   * @apiGroup Cry
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Cry'))
  /**
   * @api {delete} v4/cry/:id Delete Cry Data
   * @apiDescription Delete data related to cry with a specific ID (only accessible by admins).
   * @apiName DeleteCryData
   * @apiGroup Cry
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Cry'));
// Export the router
