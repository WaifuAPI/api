...
import getRandomShrug from '../../../controllers/v4/interactions/shrug.js';
...
router
  .route('/')
  /**
   * @api {get} v4/shrug Get a Random Shrug
   * @apiDescription Retrieve a random shrug.
   * @apiName GetRandomShrug
   * @apiGroup Shrug
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomShrug)
  /**
   * @api {post} v4/shrug Increment Shrug Data
   * @apiDescription Increment data related to shrugs (only accessible by database moderators).
   * @apiName IncrementShrugData
   * @apiGroup Shrug
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Shrug'));

router
  .route('/:id')
  /**
   * @api {patch} v4/shrug/:id Update Shrug Data
   * @apiDescription Update data related to shrugs with a specific ID (only accessible by database moderators).
   * @apiName UpdateShrugData
   * @apiGroup Shrug
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Shrug'))
  /**
   * @api {delete} v4/shrug/:id Delete Shrug Data
   * @apiDescription Delete data related to shrug with a specific ID (only accessible by admins).
   * @apiName DeleteShrugData
   * @apiGroup Shrug
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Shrug'));
// Export the router
