...
import getRandomNervous from '../../../controllers/v4/interactions/nervous.js';
...
router
  .route('/')
  /**
   * @api {get} v4/nervous Get a Random Nervous
   * @apiDescription Retrieve a random nervous.
   * @apiName GetRandomNervous
   * @apiGroup Nervous
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomNervous)
  /**
   * @api {post} v4/nervous Increment Nervous Data
   * @apiDescription Increment data related to nervouss (only accessible by database moderators).
   * @apiName IncrementNervousData
   * @apiGroup Nervous
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Nervous'));

router
  .route('/:id')
  /**
   * @api {patch} v4/nervous/:id Update Nervous Data
   * @apiDescription Update data related to nervouss with a specific ID (only accessible by database moderators).
   * @apiName UpdateNervousData
   * @apiGroup Nervous
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Nervous'))
  /**
   * @api {delete} v4/nervous/:id Delete Nervous Data
   * @apiDescription Delete data related to nervous with a specific ID (only accessible by admins).
   * @apiName DeleteNervousData
   * @apiGroup Nervous
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Nervous'));
// Export the router
