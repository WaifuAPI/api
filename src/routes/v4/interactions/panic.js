...
import getRandomPanic from '../../../controllers/v4/interactions/panic.js';
...
router
  .route('/')
  /**
   * @api {get} v4/panic Get a Random Panic
   * @apiDescription Retrieve a random panic.
   * @apiName GetRandomPanic
   * @apiGroup Panic
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomPanic)
  /**
   * @api {post} v4/panic Increment Panic Data
   * @apiDescription Increment data related to panics (only accessible by database moderators).
   * @apiName IncrementPanicData
   * @apiGroup Panic
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Panic'));

router
  .route('/:id')
  /**
   * @api {patch} v4/panic/:id Update Panic Data
   * @apiDescription Update data related to panics with a specific ID (only accessible by database moderators).
   * @apiName UpdatePanicData
   * @apiGroup Panic
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Panic'))
  /**
   * @api {delete} v4/panic/:id Delete Panic Data
   * @apiDescription Delete data related to panic with a specific ID (only accessible by admins).
   * @apiName DeletePanicData
   * @apiGroup Panic
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Panic'));
// Export the router
