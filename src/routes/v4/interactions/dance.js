...
import getRandomDance from '../../../controllers/v4/interactions/dance.js';
...
router
  .route('/')
  /**
   * @api {get} v4/dance Get a Random Dance
   * @apiDescription Retrieve a random dance.
   * @apiName GetRandomDance
   * @apiGroup Dance
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomDance)
  /**
   * @api {post} v4/dance Increment Dance Data
   * @apiDescription Increment data related to dances (only accessible by database moderators).
   * @apiName IncrementDanceData
   * @apiGroup Dance
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Dance'));

router
  .route('/:id')
  /**
   * @api {patch} v4/dance/:id Update Dance Data
   * @apiDescription Update data related to dances with a specific ID (only accessible by database moderators).
   * @apiName UpdateDanceData
   * @apiGroup Dance
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Dance'))
  /**
   * @api {delete} v4/dance/:id Delete Dance Data
   * @apiDescription Delete data related to dance with a specific ID (only accessible by admins).
   * @apiName DeleteDanceData
   * @apiGroup Dance
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Dance'));
// Export the router
