...
import getRandomTease from '../../../controllers/v4/interactions/tease.js';
...
router
  .route('/')
  /**
   * @api {get} v4/tease Get a Random Tease
   * @apiDescription Retrieve a random tease.
   * @apiName GetRandomTease
   * @apiGroup Tease
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomTease)
  /**
   * @api {post} v4/tease Increment Tease Data
   * @apiDescription Increment data related to teases (only accessible by database moderators).
   * @apiName IncrementTeaseData
   * @apiGroup Tease
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Tease'));

router
  .route('/:id')
  /**
   * @api {patch} v4/tease/:id Update Tease Data
   * @apiDescription Update data related to teases with a specific ID (only accessible by database moderators).
   * @apiName UpdateTeaseData
   * @apiGroup Tease
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Tease'))
  /**
   * @api {delete} v4/tease/:id Delete Tease Data
   * @apiDescription Delete data related to tease with a specific ID (only accessible by admins).
   * @apiName DeleteTeaseData
   * @apiGroup Tease
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Tease'));
// Export the router
