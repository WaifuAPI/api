...
import getRandomStare from '../../../controllers/v4/interactions/stare.js';
...
router
  .route('/')
  /**
   * @api {get} v4/stare Get a Random Stare
   * @apiDescription Retrieve a random stare.
   * @apiName GetRandomStare
   * @apiGroup Stare
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomStare)
  /**
   * @api {post} v4/stare Increment Stare Data
   * @apiDescription Increment data related to stares (only accessible by database moderators).
   * @apiName IncrementStareData
   * @apiGroup Stare
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Stare'));

router
  .route('/:id')
  /**
   * @api {patch} v4/stare/:id Update Stare Data
   * @apiDescription Update data related to stares with a specific ID (only accessible by database moderators).
   * @apiName UpdateStareData
   * @apiGroup Stare
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Stare'))
  /**
   * @api {delete} v4/stare/:id Delete Stare Data
   * @apiDescription Delete data related to stare with a specific ID (only accessible by admins).
   * @apiName DeleteStareData
   * @apiGroup Stare
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Stare'));
// Export the router
