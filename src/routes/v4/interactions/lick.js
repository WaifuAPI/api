...
import getRandomLick from '../../../controllers/v4/interactions/lick.js';
...
router
  .route('/')
  /**
   * @api {get} v4/lick Get a Random Lick
   * @apiDescription Retrieve a random lick.
   * @apiName GetRandomLick
   * @apiGroup Lick
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomLick)
  /**
   * @api {post} v4/lick Increment Lick Data
   * @apiDescription Increment data related to licks (only accessible by database moderators).
   * @apiName IncrementLickData
   * @apiGroup Lick
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Lick'));

router
  .route('/:id')
  /**
   * @api {patch} v4/lick/:id Update Lick Data
   * @apiDescription Update data related to licks with a specific ID (only accessible by database moderators).
   * @apiName UpdateLickData
   * @apiGroup Lick
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Lick'))
  /**
   * @api {delete} v4/lick/:id Delete Lick Data
   * @apiDescription Delete data related to lick with a specific ID (only accessible by admins).
   * @apiName DeleteLickData
   * @apiGroup Lick
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Lick'));
// Export the router
