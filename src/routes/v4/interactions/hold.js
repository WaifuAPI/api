...
import getRandomHold from '../../../controllers/v4/interactions/hold.js';
...
router
  .route('/')
  /**
   * @api {get} v4/hold Get a Random Hold
   * @apiDescription Retrieve a random hold.
   * @apiName GetRandomHold
   * @apiGroup Hold
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomHold)
  /**
   * @api {post} v4/hold Increment Hold Data
   * @apiDescription Increment data related to holds (only accessible by database moderators).
   * @apiName IncrementHoldData
   * @apiGroup Hold
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Hold'));

router
  .route('/:id')
  /**
   * @api {patch} v4/hold/:id Update Hold Data
   * @apiDescription Update data related to holds with a specific ID (only accessible by database moderators).
   * @apiName UpdateHoldData
   * @apiGroup Hold
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Hold'))
  /**
   * @api {delete} v4/hold/:id Delete Hold Data
   * @apiDescription Delete data related to hold with a specific ID (only accessible by admins).
   * @apiName DeleteHoldData
   * @apiGroup Hold
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Hold'));
// Export the router
