...
import getRandomKill from '../../../controllers/v4/interactions/kill.js';
...
router
  .route('/')
  /**
   * @api {get} v4/kill Get a Random Kill
   * @apiDescription Retrieve a random kill.
   * @apiName GetRandomKill
   * @apiGroup Kill
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomKill)
  /**
   * @api {post} v4/kill Increment Kill Data
   * @apiDescription Increment data related to kills (only accessible by database moderators).
   * @apiName IncrementKillData
   * @apiGroup Kill
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Kill'));

router
  .route('/:id')
  /**
   * @api {patch} v4/kill/:id Update Kill Data
   * @apiDescription Update data related to kills with a specific ID (only accessible by database moderators).
   * @apiName UpdateKillData
   * @apiGroup Kill
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Kill'))
  /**
   * @api {delete} v4/kill/:id Delete Kill Data
   * @apiDescription Delete data related to kill with a specific ID (only accessible by admins).
   * @apiName DeleteKillData
   * @apiGroup Kill
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Kill'));
// Export the router
