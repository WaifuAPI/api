...
import getRandomThink from '../../../controllers/v4/interactions/think.js';
...
router
  .route('/')
  /**
   * @api {get} v4/think Get a Random Think
   * @apiDescription Retrieve a random think.
   * @apiName GetRandomThink
   * @apiGroup Think
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomThink)
  /**
   * @api {post} v4/think Increment Think Data
   * @apiDescription Increment data related to thinks (only accessible by database moderators).
   * @apiName IncrementThinkData
   * @apiGroup Think
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Think'));

router
  .route('/:id')
  /**
   * @api {patch} v4/think/:id Update Think Data
   * @apiDescription Update data related to thinks with a specific ID (only accessible by database moderators).
   * @apiName UpdateThinkData
   * @apiGroup Think
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Think'))
  /**
   * @api {delete} v4/think/:id Delete Think Data
   * @apiDescription Delete data related to think with a specific ID (only accessible by admins).
   * @apiName DeleteThinkData
   * @apiGroup Think
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Think'));
// Export the router
