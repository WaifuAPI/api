...
import getRandomCheer from '../../../controllers/v4/interactions/cheer.js';
...
router
  .route('/')
  /**
   * @api {get} v4/cheer Get a Random Cheer
   * @apiDescription Retrieve a random cheer.
   * @apiName GetRandomCheer
   * @apiGroup Cheer
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomCheer)
  /**
   * @api {post} v4/cheer Increment Cheer Data
   * @apiDescription Increment data related to cheers (only accessible by database moderators).
   * @apiName IncrementCheerData
   * @apiGroup Cheer
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Cheer'));

router
  .route('/:id')
  /**
   * @api {patch} v4/cheer/:id Update Cheer Data
   * @apiDescription Update data related to cheers with a specific ID (only accessible by database moderators).
   * @apiName UpdateCheerData
   * @apiGroup Cheer
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Cheer'))
  /**
   * @api {delete} v4/cheer/:id Delete Cheer Data
   * @apiDescription Delete data related to cheer with a specific ID (only accessible by admins).
   * @apiName DeleteCheerData
   * @apiGroup Cheer
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Cheer'));
// Export the router
