...
import getRandomWave from '../../../controllers/v4/interactions/wave.js';
...
router
  .route('/')
  /**
   * @api {get} v4/wave Get a Random Wave
   * @apiDescription Retrieve a random wave.
   * @apiName GetRandomWave
   * @apiGroup Wave
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomWave)
  /**
   * @api {post} v4/wave Increment Wave Data
   * @apiDescription Increment data related to waves (only accessible by database moderators).
   * @apiName IncrementWaveData
   * @apiGroup Wave
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Wave'));

router
  .route('/:id')
  /**
   * @api {patch} v4/wave/:id Update Wave Data
   * @apiDescription Update data related to waves with a specific ID (only accessible by database moderators).
   * @apiName UpdateWaveData
   * @apiGroup Wave
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Wave'))
  /**
   * @api {delete} v4/wave/:id Delete Wave Data
   * @apiDescription Delete data related to wave with a specific ID (only accessible by admins).
   * @apiName DeleteWaveData
   * @apiGroup Wave
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Wave'));
// Export the router
