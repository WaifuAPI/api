...
import getRandomTriggered from '../../../controllers/v4/interactions/triggered.js';
...
router
  .route('/')
  /**
   * @api {get} v4/triggered Get a Random Triggered
   * @apiDescription Retrieve a random triggered.
   * @apiName GetRandomTriggered
   * @apiGroup Triggered
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomTriggered)
  /**
   * @api {post} v4/triggered Increment Triggered Data
   * @apiDescription Increment data related to triggereds (only accessible by database moderators).
   * @apiName IncrementTriggeredData
   * @apiGroup Triggered
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Triggered'));

router
  .route('/:id')
  /**
   * @api {patch} v4/triggered/:id Update Triggered Data
   * @apiDescription Update data related to triggereds with a specific ID (only accessible by database moderators).
   * @apiName UpdateTriggeredData
   * @apiGroup Triggered
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Triggered'))
  /**
   * @api {delete} v4/triggered/:id Delete Triggered Data
   * @apiDescription Delete data related to triggered with a specific ID (only accessible by admins).
   * @apiName DeleteTriggeredData
   * @apiGroup Triggered
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Triggered'));
// Export the router
