...
import getRandomDisgust from '../../../controllers/v4/interactions/disgust.js';
...
router
  .route('/')
  /**
   * @api {get} v4/disgust Get a Random Disgust
   * @apiDescription Retrieve a random disgust.
   * @apiName GetRandomDisgust
   * @apiGroup Disgust
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomDisgust)
  /**
   * @api {post} v4/disgust Increment Disgust Data
   * @apiDescription Increment data related to disgusts (only accessible by database moderators).
   * @apiName IncrementDisgustData
   * @apiGroup Disgust
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Disgust'));

router
  .route('/:id')
  /**
   * @api {patch} v4/disgust/:id Update Disgust Data
   * @apiDescription Update data related to disgusts with a specific ID (only accessible by database moderators).
   * @apiName UpdateDisgustData
   * @apiGroup Disgust
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Disgust'))
  /**
   * @api {delete} v4/disgust/:id Delete Disgust Data
   * @apiDescription Delete data related to disgust with a specific ID (only accessible by admins).
   * @apiName DeleteDisgustData
   * @apiGroup Disgust
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Disgust'));
// Export the router
