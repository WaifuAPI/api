...
import getRandomHug from '../../../controllers/v4/interactions/hug.js';
...
router
  .route('/')
  /**
   * @api {get} v4/hug Get a Random Hug
   * @apiDescription Retrieve a random hug.
   * @apiName GetRandomHug
   * @apiGroup Hug
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomHug)
  /**
   * @api {post} v4/hug Increment Hug Data
   * @apiDescription Increment data related to hugs (only accessible by database moderators).
   * @apiName IncrementHugData
   * @apiGroup Hug
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Hug'));

router
  .route('/:id')
  /**
   * @api {patch} v4/hug/:id Update Hug Data
   * @apiDescription Update data related to hugs with a specific ID (only accessible by database moderators).
   * @apiName UpdateHugData
   * @apiGroup Hug
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Hug'))
  /**
   * @api {delete} v4/hug/:id Delete Hug Data
   * @apiDescription Delete data related to hug with a specific ID (only accessible by admins).
   * @apiName DeleteHugData
   * @apiGroup Hug
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Hug'));
// Export the router
