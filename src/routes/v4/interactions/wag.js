...
import getRandomWag from '../../../controllers/v4/interactions/wag.js';
...
router
  .route('/')
  /**
   * @api {get} v4/wag Get a Random Wag
   * @apiDescription Retrieve a random wag.
   * @apiName GetRandomWag
   * @apiGroup Wag
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomWag)
  /**
   * @api {post} v4/wag Increment Wag Data
   * @apiDescription Increment data related to wags (only accessible by database moderators).
   * @apiName IncrementWagData
   * @apiGroup Wag
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Wag'));

router
  .route('/:id')
  /**
   * @api {patch} v4/wag/:id Update Wag Data
   * @apiDescription Update data related to wags with a specific ID (only accessible by database moderators).
   * @apiName UpdateWagData
   * @apiGroup Wag
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Wag'))
  /**
   * @api {delete} v4/wag/:id Delete Wag Data
   * @apiDescription Delete data related to wag with a specific ID (only accessible by admins).
   * @apiName DeleteWagData
   * @apiGroup Wag
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Wag'));
// Export the router
