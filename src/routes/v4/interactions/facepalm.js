...
import getRandomFacepalm from '../../../controllers/v4/interactions/facepalm.js';
...
router
  .route('/')
  /**
   * @api {get} v4/facepalm Get a Random Facepalm
   * @apiDescription Retrieve a random facepalm.
   * @apiName GetRandomFacepalm
   * @apiGroup Facepalm
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomFacepalm)
  /**
   * @api {post} v4/facepalm Increment Facepalm Data
   * @apiDescription Increment data related to facepalms (only accessible by database moderators).
   * @apiName IncrementFacepalmData
   * @apiGroup Facepalm
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Facepalm'));

router
  .route('/:id')
  /**
   * @api {patch} v4/facepalm/:id Update Facepalm Data
   * @apiDescription Update data related to facepalms with a specific ID (only accessible by database moderators).
   * @apiName UpdateFacepalmData
   * @apiGroup Facepalm
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Facepalm'))
  /**
   * @api {delete} v4/facepalm/:id Delete Facepalm Data
   * @apiDescription Delete data related to facepalm with a specific ID (only accessible by admins).
   * @apiName DeleteFacepalmData
   * @apiGroup Facepalm
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Facepalm'));
// Export the router
