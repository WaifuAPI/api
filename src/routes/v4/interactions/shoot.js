...
import getRandomShoot from '../../../controllers/v4/interactions/shoot.js';
...
router
  .route('/')
  /**
   * @api {get} v4/shoot Get a Random Shoot
   * @apiDescription Retrieve a random shoot.
   * @apiName GetRandomShoot
   * @apiGroup Shoot
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomShoot)
  /**
   * @api {post} v4/shoot Increment Shoot Data
   * @apiDescription Increment data related to shoots (only accessible by database moderators).
   * @apiName IncrementShootData
   * @apiGroup Shoot
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Shoot'));

router
  .route('/:id')
  /**
   * @api {patch} v4/shoot/:id Update Shoot Data
   * @apiDescription Update data related to shoots with a specific ID (only accessible by database moderators).
   * @apiName UpdateShootData
   * @apiGroup Shoot
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Shoot'))
  /**
   * @api {delete} v4/shoot/:id Delete Shoot Data
   * @apiDescription Delete data related to shoot with a specific ID (only accessible by admins).
   * @apiName DeleteShootData
   * @apiGroup Shoot
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Shoot'));
// Export the router
