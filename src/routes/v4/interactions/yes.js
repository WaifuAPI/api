...
import getRandomYes from '../../../controllers/v4/interactions/yes.js';
...
router
  .route('/')
  /**
   * @api {get} v4/yes Get a Random Yes
   * @apiDescription Retrieve a random yes.
   * @apiName GetRandomYes
   * @apiGroup Yes
   * @apiPermission user
   *
   ...
   *
   */
  .get(authorize(config.roles.USER), createRateLimiter(), getRandomYes)
  /**
   * @api {post} v4/yes Increment Yes Data
   * @apiDescription Increment data related to yess (only accessible by database moderators).
   * @apiName IncrementYesData
   * @apiGroup Yes
   ...
   *
   */
  .post(authorize(config.roles.DB_MOD), createRateLimiter(), incrementData('Yesh'));

router
  .route('/:id')
  /**
   * @api {patch} v4/yes/:id Update Yes Data
   * @apiDescription Update data related to yess with a specific ID (only accessible by database moderators).
   * @apiName UpdateYesData
   * @apiGroup Yes
   * @apiPermission database_moderator
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .patch(authorize(config.roles.DB_MOD), createRateLimiter(), updateData('Yesh'))
  /**
   * @api {delete} v4/yes/:id Delete Yes Data
   * @apiDescription Delete data related to yes with a specific ID (only accessible by admins).
   * @apiName DeleteYesData
   * @apiGroup Yes
   * @apiPermission admin
   *
   ...
   * @returns {function} Express middleware function that handles rate limiting.
   */
  .delete(authorize(config.roles.ADMIN), createRateLimiter(), deleteData('Yesh'));
// Export the router
